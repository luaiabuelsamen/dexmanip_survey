...
% --- the only reproduced plate in the paper ---------------------------------
% One reproduced plate survives in the whole paper and it is here, because it is
% the only one whose source is CC BY 4.0 and so the only one this paper has the
% right to print without asking anyone. Its caption carries the attribution the
% licence asks for. Load tex/figs/plates.tex if nothing has loaded it already.
% The \providecommand before the call makes a missing plate an empty command
% rather than an error. tex/PERMISSIONS.md is the register; nothing is pending.
% ---------------------------------------------------------------------------
\makeatletter
...
\subsection{Coupling, roles and collision}
...
\caption{Four mechanisms for controlling two dexterous hands, counted over the 28 corpus rows with
a learned closed-loop controller on two multi-fingered hands, each read from its note. The panels
are not a partition: the fourth is the same single row as the third, and the two rows whose notes
never say which architecture they use appear in neither.}
\label{fig:bimanual}
\end{figure*}

\figurename~\ref{fig:bimanual} sets the four mechanisms side by side. Every row of the 28 carries
exactly one architecture, read from its note: one policy over both hands, a network per hand,
leader and follower, or a note that does not say. The fourth panel is not a fourth architecture but
the relative frame, and it is the same single row as the third.
...
\textbf{Evidence still needed.} One architecture carries three quarters of the field, and the choice
behind it has been compared twice, with opposite outcomes, and ablated once. A decisive comparison would run
one handover task on three architectures with the same hand and the same seeds, reporting
separate giver and receiver returns rather than a single shared one.

\subsection{Benchmarks, datasets and the shared axes}
...
interpenetration as contact; \figurename~\ref{fig:contactlabel} is the chain that produces it. TACO \cite{taco_2024} gives 2.5K bimanual tool-use sequences fitted
...
\begin{figure}[!t]
  \centering
  \IfFileExists{figs/fig_contactlabel.tex}{\resizebox{\columnwidth}{!}{\input{figs/fig_contactlabel}}}{%
    \framebox[0.95\columnwidth]{\rule{0pt}{5.0cm}\textit{figs/fig\_contactlabel.tex pending}}}
  \caption{Where a capture set's contact annotation comes from. Nothing in the chain is measured
  at a surface, and a vertex that has gone through the surface is written down as a contact rather
  than as an error.}
  \label{fig:contactlabel}
\end{figure}

\textbf{Methods on the same axes.}\label{subsec:bimanual_axes}
...
% Two of the four commonest failures of two-hand grasp synthesis, at the scale of the contact.
\providecommand{\plateGraspPenetration}{}\plateGraspPenetration

Penetration, and \figurename~\ref{fig:bimanual-penetration} is why it has to be reported beside a
success rate rather than instead of one. Most bimanual rows read ``not addressed.'' BimanGrasp \cite{bimangrasp_2024} gates on
...
\subsection{Handover, and what transfers from one hand}
...
\textbf{What transfers from single-hand work.}\label{subsec:bimanual_transfer}
