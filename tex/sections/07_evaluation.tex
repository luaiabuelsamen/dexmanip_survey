...
non-zero object-orientation weight for GRAB in its Table 4, and the reward function in the
repository at the commit this survey fetched sets that orientation error to zero, so in that file
the object is tracked in position only.
...
narrower than the field and concerns learned closed-loop control: all eleven measure at the
reference rather than at the rollout, seven of them scoring a pose, a trajectory or a contact model
before anything executes and the four that are closed-loop policies scoring the references they
were given, and we found none that reports the measurement for rollouts of its own trained
policy.

\textbf{The null result was audited.} Earlier audits showed that the extraction could miss reported
values, so 25 of the 85 apparently silent methods were drawn with a fixed random seed and checked
again against their papers and repositories. None reports penetration depth, intersection volume,
or another quantitative plausibility measure on its own rollouts. With zero recoveries, a
one-sided 95 percent finite-population bound allows at most 8 hidden cases among the 85. The
reported count of eleven is therefore a lower bound and nineteen an upper bound.

GraspXL \cite{graspxl_2024} is the closest borderline case: human raters considered
interpenetration as one component of a broader realism score. We treat that as a judgement rather
than a penetration measurement. Other near misses concern self-collision, trajectory constraints,
or a solver's separation setting rather than measured hand--object overlap during a rollout. The
sample, search terms, and verdict for every audited row are included with the corpus; Appendix
\ref{app:method} gives the full procedure.
...
\textbf{Reporting penetration.} The gap is specific to learned closed-loop control rather than general,
and it is a choice rather than a capability. Evaluations should report maximum and mean penetration
...
The reproducibility materials give the full derivation; its conclusions are as follows, and every
...
\cite{dextreme_2022}, for example, reports a success criterion and a confidence interval, but
uses ten trials. This limits retrospective comparison; it does not imply that the reported
experiments failed their own stated objectives.

\textbf{What would have to be true.}\label{subsec:what_would_be_true}
The protocol is expensive. For each policy and task it calls for 57 matched anchor trials and 100
unseen-object trials. Comparing two policies on three tasks therefore requires 942 real rollouts,
or about 16 hours at one minute per rollout before repairs and scoring. Pairing saves roughly a
fifth relative to an independent design because each pair still requires two rollouts. AutoEval
\cite{autoeval_2025} completed about 850 WidowX episodes in 24 hours with three interventions and
regular cooling pauses; a tendon-driven hand is likely to stretch the same workload across several
days.
...
The proposal has four limits. It has not been run, and its sample sizes come from power and interval
calculations rather than dexterous-hand experiments. The worst-case calculation assumes a success
rate of 0.5, while the paired design assumes a discordance rate of 0.3 that the literature does not
report. Its perturbations come from parallel-jaw benchmarks \cite{colosseum_2024,simpler_2024},
where contact is simpler. Finally, the 2 mm penetration threshold follows
\cite{toporetarget_2026}; it is a simulator convention, not a physical limit. Captured human
grasps in GRAB \cite{grab_2020} average 3.25 mm under the same kind of measurement. Establishing a
meaningful threshold will require measurements across hands, engines, and accepted behaviours.
...
above is therefore assembled from work on grippers and on whole-arm tasks. A modest first test would
run the protocol in Table~\ref{tab:protocol} once, on one in-hand reorientation
task with one 16-DoF hand, and release the rollouts and per-trial outcomes for independent analysis.
