...
No degree-of-freedom, force, weight or price figure in Table~\ref{tab:hands-available} or Table~\ref{tab:hands-announced} was measured by anyone
outside the maker. Six rows are the exception, from peer-reviewed papers with stated protocols:
ILDA, Pisa/IIT, ORCA, RUKA, LEAP and BiDexHand. Table~\ref{tab:hands-available} dates every other claim.
...
protocol \cite{orca_hand_2025}. No two hands in Table~\ref{tab:hands-available} report payload under the same test either.
...
\cite{leap_hand_2023}. A reader deploying torque control is bitten by that first. The second column says
...
% ---------------------------------------------------------------------------
% Table 2, the hands that can be bought or built. Generated from the rows of
% class "hand" into tex/tables/; the file carries its own float, caption and
% \label{tab:hands-available}. The guard lets this section compile before the
% generator has run.
% ---------------------------------------------------------------------------
\IfFileExists{tables/table2_hands_available.tex}{\input{tables/table2_hands_available}}{%
  \IfFileExists{tables/table1_hands_available.tex}{\input{tables/table1_hands_available}}{%
    \begin{table*}[!t]\centering\framebox[0.7\textwidth]{\rule{0pt}{1.6cm}%%
    \textit{tables/table2\_hands\_available.tex pending}}\end{table*}}}
...
\begin{figure}[!t]
  ...
  whose own experiments use it, over the 103 of the 112 method rows that name a hand; a paper
  using two hands is counted under both. The accented bar is the parallel-jaw gripper, which is
  not a dexterous hand, and only three dexterous hands carry more method rows than it. Only
...
Seven rows in Table~\ref{tab:hands-available} are open hardware. Five state a dollar cost in the price column, ORCA states
a material cost below 2,000 CHF that the column leaves unconverted, and BiDexHand states none
\cite{orca_hand_2025,bidexhand_2025}. The Faive Hand states neither a cost nor a licence that could be
...
% ---------------------------------------------------------------------------
% Table 3, the announced and unreleased hands, carrying \label{tab:hands-announced}.
% ---------------------------------------------------------------------------
\IfFileExists{tables/table3_hands_announced.tex}{\input{tables/table3_hands_announced}}{%
  \IfFileExists{tables/table2_hands_announced.tex}{\input{tables/table2_hands_announced}}{%
    \begin{table*}[!t]\centering\framebox[0.7\textwidth]{\rule{0pt}{1.6cm}%%
    \textit{tables/table3\_hands\_announced.tex pending}}\end{table*}}}
...
Almost none of the eight uses a high-resolution sensor. Yin et al. \cite{rotating_without_seeing_2023} removes
...
resistors read as binary \cite{robot_synesthesia_2023}. Two papers exclude touch deliberately, HORA
reporting rotation ``even without the usage of vision and tactile sensing'' \cite{hora_2022}, and ORCA's
authors dropping it from their reinforcement learning ``due to the additional complexity involved in
accurately modeling them'' \cite{orca_hand_2025}. Taxel counts have risen by three orders of magnitude
...
Table~\ref{tab:hands-announced}'s emptiness is measurable and part of the same finding. Over the twelve specification
...
The gap runs the other way too, and the last column of Table~\ref{tab:hands-available} shows it. Seven hands that are sold
and documented take zero method rows each: the Unitree Dex5 with 94 pressure sensors on its P
variant \cite{unitree_dex5_2025}, the fully actuated Tesollo DG-5F \cite{tesollo_dg5f_2024}, ORCA, RUKA,
Ruka-v2, BiDexHand and DexHand. Cheapness is established only for RUKA, Ruka-v2 and DexHand,
because Table~\ref{tab:hands-available}'s price cell is empty for Unitree, Tesollo, ORCA and BiDexHand, and the only Dex5
