...
\begin{figure}[!t]
  ...
  survey's fault rather than theirs. Of the 112 method rows, 62 released code that could be read
  against the paper and 38 of those record a disagreement. The lower bar reopens those 38 at their
  own scale: 10 are contradictions, where the shipped code states a different objective from the
  published one, and 28 are withdrawn, over half of them limits of this survey's own parse rather
  than findings about the work. Seven further accusations an earlier draft made were withdrawn under
  adversarial review, each recorded in the accused row's \texttt{mismatch\_review} field.
  Section~\ref{sec:train:code} classifies all 38 and names them.}
  \label{fig:codegap}
\end{figure}
...
this survey adds are columns nobody else fills. The first is
