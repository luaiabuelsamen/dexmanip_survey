...
\subsection{The full method table}
Table~\ref{tab:app-methods} is the long form of the method table the body prints in truncated
form. It carries one row for each of the 112 method rows, at full width, so that the cells the
body table has to cut, among them the hand and its degrees of freedom, the simulator, the real
trial count and the code release, can be read for every method at once. It is generated from the
same structured rows as every other table here, which is why a cell can be empty but cannot hold
a value no note confirmed.

%% tables/methods_full.tex is generated into tex/tables/ by the table generator in tools/.
%% Do not write the table out here; regenerate it there.
\IfFileExists{tables/methods_full.tex}{\input{tables/methods_full.tex}}{%
  \begin{table}[!t]
  \caption{The full method table, one row per method row, at full width. Pending: generated into \texttt{tex/tables/methods\_full.tex} by the table generator in \texttt{tools/}.}
