...
corpus entry it came from and to the page it was taken from. Eighteen are reproduced, and they
stand in two positions. Seven are cleared: each source carries CC BY 4.0, read directly from the
licence page on the date the register records, and those seven captions print the attribution the
licence asks for, naming the creator, the work, the licence with a link and the change made to the
plate. The other eleven are permission pending and their captions say so: two need a request
through a publisher's own process and nine need an email to the authors, arXiv's non-exclusive
licence to distribute granting a third party nothing. The register that records, per figure, the
licence found, the URL it was read from and the action still open, is \texttt{tex/PERMISSIONS.md}.
The build does not enforce that register.
