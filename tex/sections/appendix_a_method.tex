...
nulls and moved the ``never says'' figure from 34 of 89 down to 19. The same mechanism reaches the
penetration, code-release and failure-mode fields, none of which has been audited that way. Read
...
results matrix. The sixth is Table~\ref{tab:surveys}, the comparison to the fourteen predecessor
surveys, which is this survey's positioning and is therefore set where a reader meets it rather
than in the repository; it prints one of three levels a cell, and Appendix~\ref{app:surveys} keeps the
sentence each level was read from.

\textbf{Reproduced figures.}
Three figures in this edition are not this survey's own artwork. They are photographs and renders
reproduced from the papers they discuss, rendered from the stored PDFs in the corpus, and each one
states its source in its own caption, keyed to the corpus entry it came from and to the page it
was taken from. An earlier draft reproduced eighteen. A plate is kept only where it carries
something a drawing cannot: that these hands are that much larger than a human one, and that a
contact defect invisible at the scale of a grasp is unmistakable at the scale of the contact.
The rest were redrawn in the paper's own style or dropped, and what each one was standing in for
is said in the comment at the head of the figure that replaced it.
One of the three is cleared: its source carries CC BY 4.0, read directly from the licence page on
the date the register records, and its caption prints the attribution the licence asks for, naming
the creator, the work, the licence with a link and the change made to the plate. The other two are
permission pending, both needing an email to the authors, arXiv's non-exclusive licence to
distribute granting a third party nothing. No caption prints that status, because a permission
still to be sought is a working note and not a finding. The register that records, per figure, the
licence found, the URL it was read from and the action still open, is \path{tex/PERMISSIONS.md}.
