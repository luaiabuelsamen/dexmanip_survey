...
complete one cycle. Section~V-D's contact-graph term exists because of the general shape of the
...
behind these headlines are small enough that Section~VII-A treats them as the reporting problem
they are.
...
of the reference and not of the behaviour.
...
reinforcement-learning literature of Section~V-B works in, and the two rows that reach it on paper
...
GPU-days of Section~V-B, and it is the only method here that imposes non-penetration as a hard
...
in Section~V-B now reaches the shipped policy through a dataset rather than a gradient.
...
charge. Among the 21 reorientation methods scored term by term, seven released a repository: four
disagree with their paper, one \cite{dreureka_2024} ships no cube-rotation environment at all, one
\cite{hora_2022} is a later generation its own README flags, and one \cite{eureka_2023} was a
default-value question the same README settles.
...
rotation and body rotation-velocity still live. Omnigrasp \cite{omnigrasp_2024}, in the same file
family, keeps its object rotation term live and fails the other way, internally:
\texttt{compute\_pregrasp\_reward\_time} takes its weights as arguments and then hardcodes them to
0.9 and 0.1.
...
DexPBT \cite{dexpbt_2023} sums eight components against the paper's four and multiplies a
hand-delta penalty by zero with the comment ``currently disabled'', a term the paper never claims,
and its five named weights are present at their stated values. PianoMime \cite{pianomime_2024}'s
Table~3 states two weighted terms while its environment sums roughly five unweighted ones, two of
them inherited stubs returning zero and a third, forearm collision, the paper never lists. PenSpin
\cite{penspin_2024} ships \texttt{forceScale: 0.0} against the disturbance force its appendix
describes, which is the charge that survives. A second half of the original charge, that the same
96-dimensional observation disables the paper's tactile channel, is withdrawn: those dimensions
are proprioception-only, consistent with the student policy the released config runs rather than
the tactile-and-point-cloud oracle, and the paper never claims the student has tactile input. The
disturbance-force charge alone is why the row still reads medium rather than high.
...
\cite{visual_dexterity_2022}'s Eq.~8 penultimate-joint penalty is absent from the released reward
file, and its two configs disagree about the fall distance. UniDexGrasp \cite{unidexgrasp_2023}
and DexPoint \cite{dexpoint_2022} ship rewards structured differently from their equations, and
...
fallback. Hora \cite{hora_2022} is the one repository that discloses its own gap, telling the
reader to check out tag v0.0.1. Elsewhere a paper disagrees with itself: ALOHA
\cite{aloha_act_2023}'s Algorithm~1 says MSE and its Section~IV.C says L1, \cite{dp3_2024}'s prose
says the network predicts noise while its config sets \texttt{prediction\_type: sample}, and
DexMachina \cite{dexmachina_2025}'s multiplicative task reward, charged to its code in an earlier
draft, is printed in the paper.
...
ones, after Section~VII-A recovered 15 trial counts and 7 unseen-object counts that the notes
carried and the extraction had dropped. A mostly empty row is not a weak method, but it is one
that cannot be compared with any other row here. All 112 rows carry the columns this paragraph
