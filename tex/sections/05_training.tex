...
Hora \cite{hora_2022} states a joint-noise range of U(0, 0.005), while its public configuration
uses 0.02; its README warns that this revision does not reproduce the paper. PenSpin
\cite{penspin_2024} describes random disturbance forces, while the only public configuration that
names them disables them. DexPBT \cite{dexpbt_2023} is consistent: its configuration disables
randomisation, as the paper says.
...
\textbf{Cross-hand retargeting.} Human data does not port across hands, and
...
\cite{egozero_2025}. A shared benchmark could make these mappings comparable. TopoRetarget's
format \cite{toporetarget_2026} is a useful starting point: it reports contact precision,
contact alignment, maximum penetration and
...
\textbf{Dexterous hands in generalist policies.} The gap is size rather than absence. The
...
smaller half of the same gap. The standard
generalist suite, with the hand's actuated degrees of freedom and its vendor printed beside the
number, should include at least one dexterous-hand task.
...
% retracted; the seven rows the survey withdrew are counted in section
% 8.1 and in each row's mismatch_review field. Written by
...
Nine is the number to quote, eight at high confidence and one, PenSpin \cite{penspin_2024},
held at medium. Nine of 62 is 15 percent, and it is a floor rather than a rate: the census
covers method rows only, so RoboPianist \cite{robopianist_2023}, whose row is a benchmark, does
not enter it although it sums five reward terms against the three its Table~2 documents. It is
also a screened count rather than a first pass. Sixteen rows were drawn at first and seven have
since left the class, six under adversarial re-reading and one, OmniH2O \cite{omnih2o_2024},
once writing to its authors sent someone back to the evidence; two more were narrowed and
stayed, and each revision is recorded in the row it concerns beside the comparison it revises.
That screening is evidence that the nine were checked, not a bound on how many a fuller reading
would find. Among the 21 reorientation methods scored term by term, seven released a repository
and four of those state something different from their paper, the other three being a missing
environment, a later generation and a default-value question their own READMEs settle.

PhysHOI \cite{physhoi_2023} is the clearest example. Its paper assigns nonzero weights to object
rotation and rotation velocity on the GRAB dataset. In the public implementation, both errors are
set to zero and the calculations are commented out. The paper does specify zero weights for the
separate BallPlay task, but the implementation does not condition this choice on the dataset. Its
reported success criterion uses position alone, so that score cannot expose the difference. The
claim here is limited to this conflict between the paper and the public revision listed in
Table~\ref{tab:codegap}; it does not establish which code produced the published experiments.

The other cases vary in importance. DeXtreme \cite{dextreme_2022} gives one action-change penalty
in the paper and a different value in each of two public configurations. PianoMime
\cite{pianomime_2024} describes two reward terms, while its implementation sums five and forces
two of them to return zero. Table~\ref{tab:codegap} gives the exact file, revision, and values for
these and the remaining six cases, where they can be checked without interrupting the argument
with source-code identifiers.

\textbf{Scope and correction.} The authors of the nine works were not contacted before this survey
was posted. The comparison is therefore limited to two public artefacts: the paper and the
repository revision named in Table~\ref{tab:codegap}. It makes no claim about intent or about the
private code used for the reported experiments: a fetched commit is not the code that produced a
paper's numbers. It may predate, postdate, or differ from that code. Hora \cite{hora_2022} says
exactly that about itself by directing readers to a tagged release rather than its default branch.
The letters in \path{outreach/} remain unsent and preserve the evidence that would have been sent
to each author. Every comparison is correctable in public through the address in the author block;
supporting evidence changes the record, the derived counts, and the next version of the text.

The classification distinguishes a term that is absent, present with zero weight, or assigned a
different weight. In 13 cases the parsed repository cannot settle the comparison, and those cases
are recorded as limitations rather than conflicts. The reproducibility materials preserve all 38
records, including the file, both values, confidence, and review history.

\textbf{Review and correction.} This is a result about publishing practice in
robot learning rather than about dexterous manipulation in particular: the dexterous corpus is its
sample, not its subject. Nine is a lower bound because 46 method papers released no code to inspect
and four comparisons remain unsettled. It is also the result of a conservative review. The
first draft identified sixteen conflicts; seven have since left the count and two more were
narrowed and remain in it. Six failed under adversarial rereading, and a seventh was
reclassified when its apparent weight mismatch proved consistent with a curriculum factor.
PenSpin \cite{penspin_2024}, the sole
medium-confidence case retained, concerns a disabled disturbance rather than the tactile input
claimed in an earlier draft. Each change and its reason is preserved with the underlying record.
Future papers can avoid most of this ambiguity by generating the reward table directly from the
configuration used for training and citing the corresponding revision.
