<!-- page 1 (ocr) -->
arXiv:2312.07843v1  [cs.RO]  13 Dec 2023
1
...
are trained on small datasets tailored for specific tasks, which
...
display an emergent ability to find zero-shot solutions to problems
...
enable open-vocabulary visual recognition. However, significant
...
quantification, and real-time execution. In this survey, we study
...
on
extensive
internet-scale data and can be fine-tuned for adaptation to
a wide range of downstream tasks. Foundation models have
demonstrated significant breakthroughs in vision and language
...
industrial robotics, assistive robotics, medical robotics, field
...
significantly expand adaptability, generalization capability, and
overall performance. Ultimately, foundation models may hold
the potential to yield these same benefits in robotics. Knowl-
edge transfer from foundation models may reduce training
time and computational resources compared to task-specific
...
training data for that specific task. The would enable robots to
generalize their learned knowledge to novel cases, enhancing
adaptability and flexibility for robots in unstructured settings.
...
tual data, aiding tasks such as zero-shot image classification,
zero-shot object detection [10], and 3D classification [11].
...
awareness by associating words with specific objects, loca-
tions, or actions within the 3D environment.
In the decision-making or planning domain, LLMs and
VLMs have been found to assist robots in task specifica-
...
possibility to improve data efficiency and enhance contextual
...
in vision and language processing, the generalization and fine-
...
dation model, 3) Uncertainty Quantification: how to deal
...
contrast to some existing surveys that focus on a specific in-
...
progress and existing deficiencies in the research, and point
...
papers that adapt or fine-tune foundation models for
robotic systems, or papers that build new robotic-specific
foundation models.
...
purposes. Next, robot task specification and code generation
...
scene representation, zero-shot 3D classification, affordance
...
training process demands significant computational resources,
requiring specialized hardware such as GPUs or TPUs, as
well as software and infrastructure for model training which
requires financial resources. Additionally, training a foundation
...
In this section, we first introduce common terminologies in
...
for regression or classification tasks. In contrast to genera-
...
values weighted by attention weights,
j αijvj. One of the
key reasons for the success of the trans
er attention model
is that it can be efficiently computed with GPUs and TPUs by
...
transformer.
The size of a transformer model is typically quantified by
...
been applied in many fields as a representation of random
...
in the sequence, repeating this process indefinitely. Classical
...
concepts were adapted to deep learning models first with
...
transformer model by using a modification introduced in [82]
...
by ConVIRT [83] and first presented in prior works [84]–[87].
...
final loss is an average of the two loss components where each
...
with 130 billion parameters. LLMs can also be fine-tuned,
a process by which the model parameters are adjusted with
domain-specific data to align the performance of the LLM
to a specific use case. For example, GPT-3 and GPT-4 [3]
have been fine-tuned using reinforcement learning with human
feedback (RLHF).
...
sification segmentation, and object detection. A ViT treats an
image as a sequence of image patches referred to as tokens.
In the image tokenization process, an image is divided into
patches of fixed size. Then the patches are flattened into a one-
...
of parameters. Given a fixed teacher network, the student
...
(a modified version of vision transformer ViT) that are trained
...
achieving finer-level alignment in multimodal learning. It
...
information. FLIP [111] proposes a simple and more efficient
training method for CLIP. FLIP randomly masks out and
removes a significant portion of image patches during training.
This approach aims to improve the training efficiency of CLIP
...
language embedding space via an affine transformation. The
...
of task specification, allowing robots to receive and interpret
high-level instructions from humans. VLMs may also promise
contributions to this field. VLMs specialize in the analysis of
...
making, as robots can refine their actions based on the
...
instruction l ∈L. The loss function is defined as the maximum
...
main challenges in this domain are: (i) obtaining a sufficient
...
plan to perform the task specified by the user. In addition, the
...
data can be used for policy fine-tuning. CLIPort [25] also
...
work solves language-specified manipulation tasks without
...
of voxel patches for efficient language-conditioned behavioral
...
the MAE encoder on a language prefix. Temporal information
...
out-of-domain data to improve training efficiency. Finally, in
...
<!-- page 11 (ocr) -->
11
forms of task specification. Notably, MimicPlay [118] presents
...
these two data sources, this algorithm can significantly reduce
...
time. MUTEX [119] further explores learning a unified policy
across multimodal task specifications in video, image, text,
...
Fast and flexible adaptation is a desired capability of ar-
tificial agents and is essential for progress toward general
...
exploration to refine its policy toward optimal performance.
...
XL [121] with some modification is used to enable long
...
fied RL framework. This work considers robot manipulation
...
for zero-shot reward specification. The reward model is derived
...
is implicitly defined via distance embedding. The proposed
...
For tasks specified as language goals or image goals, a multi-
...
with text annotation, and the model can be fine-tuned on
small datasets of in-domain robot data to ground language
in a context-specific way. LIV uses a generalization of the
...
framework to multi-modal goal specifications. LOREL [38]
learns a language-conditioned reward from offline data and
...
with specific locations and objects in its surroundings. In
...
world-grounding (Can), enabling the identification of feasible
...
specific feedback, such as success detection, and scene-specific
...
time step. The task planning problem is to find a sequence of
...
geometric feasibility score (Sgeo) is defined as the probability
...
conditioned robot skills from unstructured offline data in the
...
1) Language Instructions for Task Specification: As dis-
...
Temporal logic is useful for imposing temporal specifi-
...
is finetuned using the dataset. LLMs are often used to plan
...
<!-- page 13 (ocr) -->
13
prompt to the LLM includes specifications of the available
...
sively defining undefined functions. This enables the LLMs to
generate more complex code structures to fulfill the desired
policy requirements.
...
multiple atomic tasks executable by the robot is defined. Then,
a prompt is crafted that includes these functions, and the
required constraints along the task description. ChatGPT then
provides executable code specific to the given robot configura-
tion and task. The generated code can then be evaluated by a
user and appropriate feedback with modified prompts to LLMs
further help refine and generate programs that are safe and
...
and finds extensive use in natural language processing. The
method of Chain-of-Thought [131] is a prominent technique
within in-context learning. It involves executing a sequence of
intermediate steps to arrive at the final solution for complex,
multi-step problems. This technique allows models to pro-
duce step-by-step explanations that parallel human cognitive
processes. However, despite its numerous benefits, ICL also
faces certain challenges, including issues related to ambiguity
and interpretation, domain-specific knowledge, transparency,
and explainability. In-context learning has had a significant
impact on the field of LLMs in a broad sense, and many
robotics works have used it to apply LLMs to specific domains.
...
allows for the application of LLMs in scenarios such as offline
...
to identify specific brief sequences within demonstrations,
...
that without any task-specific fine-tuning of the pretrained
...
This work considers robot manipulation tasks. They find that
...
fleet of 13 robots. RT-1 receives images and natural language
...
use pre-existing vision-language models and directly co-fine-
...
robot policy. To make co-fine-tuning possible, the actions are
...
this unified dataset, RT-X demonstrates that RT-1- and RT-2-
...
captures dynamics and behaviors specific to a robot. PACT
was tested in experiments involving mobile agents: a wheeled
robot with a LiDAR sensor (MuSHR) and a simulated agent
using first-person RGB images (Habitat). The results show
that this robot-specific representation can serve as a starting
point for tasks like safe navigation, localization, and mapping.
Additionally, the experiments demonstrated that fine-tuning
smaller task-specific networks on the pre-trained model leads
to significantly better performance compared to training a
...
transformers, providing a pretraining-finetuning approach tai-
...
across various tasks. Subsequently, the finetuning process can
adapt to a wide variety of tasks spanning diverse domains. Ex-
perimentation underscores SMART’s ability to enhance learn-
ing efficiency across tasks and domains. This work considers
...
configuration, language instructions from a user, and images of
each object in the environment. The model’s output is modified
for each waypoint in the trajectory so that the final robot
...
robots in zero-shot. It can be fine-tuned to be adapted for
...
visual goals using an EfficientNet CNN and predicts temporal
...
to find the object described by humans and navigate towards
...
uses an open-vocabulary classifier such as CLIP [4] to compute
the cosine similarity between an image and a user-specified
description.
...
as RoBERTa-large [149], enabling efficient exploration and
...
is the problem of finding an object in any unseen environment,
...
fications that combines textual and visual tokens. Multi-modal
...
It offers the training of a unified policy across diverse tasks,
...
instructions to construct a goal configuration for objects that
were previously seen or unseen. They accomplish this by
first using segmentation to break up the scene into objects.
Then they use a multi-model transformer to combine word
and point cloud embeddings and output a 6-DoF goal pose
prediction. The predictions are iteratively refined via diffusion
and a discriminator that is trained to determine if a sampled
configuration is feasible. Manipulation of Open-World Objects
...
arrangement.
In Table II some robotic-specific foundation models are re-
...
sensory inputs into a unified representation encompassing
semantic, spatial, temporal, and affordance information. These
multi-modal models reflect cross-modal interactions, often by
...
redefining object detection as phrase grounding. This refor-
...
ViNT [137]
EfficientNet+ decoder transformer
31M
visual navigation
4Hz
variety of GPU configurations is
...
to super points, and finally groups super points to obtain
...
image-text pre-training and detection end-to-end fine-tuning.
...
vocabulary object detector to find “entities of interest” (e.g.,
vase or drawer handles) and ultimately define value maps for
...
at multiple stages.
2) 3D Classification: Zero-shot 3D classifiers can enable
...
performing 3D classification. PointCLIP [54] transfers CLIP’s
pre-trained knowledge of 2D images to 3D point cloud un-
derstanding by aligning point clouds with text. The authors
propose to project each point onto a series of pre-defined
...
final prediction for the point cloud is computed via weighted
...
is a Unified representation of Language, Images, and Point
...
using a unified multimodal representation of ULIP.
B. Open-Vocabulary Semantic Segmentation
Semantic segmentation classifies each pixel in an image
into semantic classes. This provides fine-grained information
...
alizability and flexibility of semantic segmentation tasks.
LSeg
is
a
language-driven
...
work for promptable segmentation consisting of the task defi-
...
periments on five zero-shot transfer tasks, including point-valid
...
specifically for fixed task sets. However, one limitation of this
...
3D scenes.
Amidst these remarkable advancements, achieving fine-
grained detection with real-time performance still remains
challenging. For example, LSeg [58] reports failure cases
related to misclassification, when the test time input labels
...
be defined as labels, but during inference, a pixel representing
a “window” may be labeled instead as “house”. SAM also does
not provide precise segmentation for fine structures and often
fails to produce crisp boundaries. All models that use SAM
as a sub-component may encounter similar limitations. In the
future, fine-grained semantic segmentation models that can
...
NeRF-based models memorize the light field in a single
...
Kerr et al. [64] propose language-embedded radiance fields
(LERFs) that ground CLIP embeddings into a dense multi-
scale 3D field. This results in a 3D representation of the
...
and finally, the CLIP embeddings are supervised through
...
and NeRF. For example, CLIP exhibits difficulty in capturing
spatial relationships between objects. In addition, language
queries from CLIP can highlight a significant issue similar to
...
latent vector to different modality-specific outputs. The model
...
segmentation, instance identification, semantic search over
...
modules are trained irrespective of the specific object labels,
...
overcome 3D data scarcity, the authors propose an efficient
...
vocabularies with 3D location tokens. In the first part, the
...
fields. CLIP-NeRF facilitates the editing of the shape and
...
fields via volume rendering. Editing is achieved by alpha
compositing the density and color values of the two NeRF
scenes. When combined with CLIP-NeRF, this method enables
CLIP-NeRF to selectively edit specific regions of multi-object
...
use structured 3D scene representations. Nerflets [175] repre-
sent a 3D scene as a combination of local neural radiance fields
...
colors and densities as standard NeRF, individual Nerflets are
combined to predict these values, modulated by their weights.
After optimizing posed 2D images and segmentations, Nerflets
reflect the decomposed scene and support more controlled
editing.
...
robust policies; however, generating images with sufficient
...
Another direction is to use generative models to define
goal images for planning. DALL-E-Bot [157] uses DALL-E
2 to define a goal image of human-like arrangements from
observations.
...
descriptor fields (NDFs) [179] remove the need for dense
...
finding corresponding object regions. F3RM uses a similar
...
[181] without training. This method first extracts dense ViT
...
or entities to offer specific functions or interactions to an agent.
...
different robot learning paradigms. Firstly, they apply offline
...
the robot to learn how to achieve specific objectives by
...
Another example is COMPASS [160], which first constructs
...
bodiedGPT [70] utilizes prefix adapters to augment the 7B
...
to make progress toward solving tasks specified in natural
language.
...
quire model parameter fine-tuning. Exploration is maximized
...
by Minecraft, execution errors, and a self-verification scheme.
For self-verification, GPT-4 acts as a critic by checking task
...
human critic to provide on-the-fly human feedback during task
...
world robotics by providing efficient training and optimizing
control policies in a safe and controlled setting. Reward design
is a crucial aspect of RL that influences the robot’s learning
...
investigate the simplification of reward design by utilizing
...
offline reinforcement learning and language modeling. They
...
different offline RL tasks and assess their performance on
various language-related tasks. Tarasov et al. [199] present
an approach to harness pretrained language models in deep
offline reinforcement learning scenarios that are not inherently
compatible with textual representations. The authors suggest a
method that involves transforming the RL states into human-
readable text and performing fine-tuning of the pretrained lan-
guage model during training with deep offline RL algorithms.
...
vision and language. They specifically focus on the effec-
tiveness of fine-tuning these pretrained models on offline RL
...
environment. In this work, first, an autoregressive LLM such
...
tion. ReAct showcases its proficiency across a wide array
of language and decision-making tasks, including question-
answering and fact verification. It enhances interpretability and
...
and can be fine-tuned for more complex skills using imitation
...
iments. Gato has 1.2B parameters and is trained offline in a
supervised way. Positioned at the confluence of representation
...
RoboTHOR include its reconfigurability and benchmarking
capabilities. The physical environments are constructed using
modular, movable components, enabling the creation of di-
verse scene layouts and furniture configurations in a single
...
specific data is scarce. We discuss various techniques to over-
...
with significant costs and potential safety concerns. One
...
generate sufficient semantically and visually diverse data,
...
or shape generation, language-driven 3D classification, and
...
High-fidelity simulation via gaming engines can provide an
efficient means to collect data, especially to solve multimodal
...
to label offline datasets for language-conditioned policy learn-
ing. DIAL performs instruction augmentation using VLMs to
weakly relabel offline control datasets. DIAL consists of three
steps 1) Contrastive fine-tuning of a VLM such as CLIP [4] on
a small robot manipulation dataset of trajectories with crowd-
sourced annotation, 2) producing new instruction labels by
using the fine-tuned VLM to score relevancy of crowd-sourced
...
improve the computational efficiency of foundation models.
...
be downloaded and run locally with sufficient local computing
...
embedding is assumed to be sufficient to identify the modality
...
sufficient data is not available and to be able to align them
with other modalities, they need to be first converted to other
...
challenge is first converting this 3D point cloud data to other
...
over object labels. Instance-level uncertainty quantification can
inform the robot’s decisions at runtime. For example, if an
image classification model running on an autonomous vehicle
...
quantification using prediction sets, calibration asks for the
prediction set to contain the true label with a user-specified
probability (e.g., 95%) over future inputs. If instance-level
uncertainty is quantified using a distribution over outputs, it
should be the case that outputs that are assigned confidence p
...
looked when performing uncertainty quantification in robotics:
...
depend on the specific prior that is used to produce the
...
calibrated uncertainty quantification is distribution shift. A
...
model; since the robot’s actions influence future states and
...
trained on.
5) Case Study: Uncertainty Quantification for Language-
Instructed Robots: Recently, there has been exciting progress
in performing rigorous uncertainty quantification for language-
...
or clarification from humans in order to resolve uncertainty.
KNOWNO performs both instance-level and distribution-level
uncertainty quantification in a calibrated manner using the
...
task success (i.e., distribution-level uncertainty quantification).
...
instance-level and distribution-level uncertainty quantification
...
quantification techniques (e.g., ensembling [217], [218]) can
...
uncertainty quantification. How can we rigorously test for the
...
uncover flaws. Third, foundation models such as LLMs are
...
does so for current field-deployed systems such as autonomous
vehicles [222], [223]. The primary challenges are to ensure
that (i) the simulator has high enough fidelity for results to
meaningfully transfer to the real world, and (ii) test scenarios
(manually specified, replicated from real-world scenarios, or
automatically generated via adversarial methods [224]) are
representative of real-world scenarios and are diverse enough
to expose flaws in the underlying foundation models. In
addition, finding ways to augment large-scale simulation-based
testing with smaller-scale real-world testing is an important
direction for future work. We emphasize the need for perform-
ing such testing throughout the lifecycle of a field-deployed
...
In addition to performing rigorous testing offline, robots with
...
error rates in a data-efficient manner remains an important
research direction.
...
approach simplifies and facilitates the integration of recent
...
and scalability, they are not always customized to specific
applications. When specific domain expertise is needed, it is
necessary to build a foundation model from scratch or fine-
tune the existing models. Building a foundation model from
scratch is resource-intensive and demands significant compu-
tational power. However, it provides fine-grained control over
...
physical characteristics, configurations, and capabilities. Real-
...
and objects for specific tasks. These solutions are not gener-
...
Another significant obstacle in incorporating foundation
...
tasks with high visual fidelity. This leads to a large sim-to-
...
and simulation setups can significantly alleviate the challenges
...
the concepts in that research field to real-world robotic applica-
...
settings, uncertainty quantification, safety evaluation, and real-
time performance remain significant concerns that demand
future research. We have delved into some of these challenges
and have discussed potential avenues for improvement.
ACKNOWLEDGMENTS
The first author was supported on an ASEE e-Fellows
...
REER Award [#2044149] and the Office of Naval Research
[N00014-23-1-2148].
REFERENCES
...
[13] Sherry Yang, Ofir Nachum, Yilun Du, Jason Wei, Pieter Abbeel, and
...
Wulfmeier, Nicolas Heess, and Martin Riedmiller. Towards a unified
...
Zero-shot
reward specification via grounded natural language. In ICML, pages
...
offline data and crowd-sourced annotation. In CoRL, pages 1303–1315.
PMLR, 08–11 Nov 2022.
...
ULIP: Learning unified representation of language, image and point
...
radiance fields. In CVPR, pages 3835–3844, 2022.
[64] Justin Kerr, Chung Min Kim, Ken Goldberg, Angjoo Kanazawa, and
Matthew Tancik. LERF: Language embedded radiance fields. In ICCV,
pages 19729–19739, 2023.
[65] Sosuke Kobayashi, Eiichi Matsumoto, and Vincent Sitzmann. Decom-
posing NeRF for editing via feature field distillation. In NeurIPS, 2022.
[66] Yufei Ye, Xueting Li, Abhinav Gupta, Shalini De Mello, Stan Birch-
field, Jiaming Song, Shubham Tulsiani, and Sifei Liu.
Affordance
...
unified policies from multimodal task specifications. In CoRL, 2023.
...
based object localization within neural radiance fields. IEEE Robotics
...
[170] Nur Muhammad Mahi Shafiullah, Chris Paxton, Lerrel Pinto, Soumith
Chintala, and Arthur Szlam. CLIP-Fields: Weakly supervised semantic
fields for robotic memory. In RSS, 2023.
...
Guibas, Hao Su, and Kyle Genova. Nerflets: Local radiance fields for
efficient structure-aware 3d scene representation from 2d supervision.
...
and Phillip Isola. Distilled feature fields enable few-shot manipulation.
In CoRL, 2023.
...
Self-supervised traffic advisors: Distributed, multi-view traffic predic-
...
[192] Yilun Du, Mengjiao Yang, Bo Dai, Hanjun Dai, Ofir Nachum, Joshua B
...
2023.
[198] Ziqi Zhang, Yile Wang, Yue Zhang, and Donglin Wang. Can offline
...
and pre-trained language models for offline reinforcement learning. In
...
help offline reinforcement learning? arXiv preprint arXiv:2201.12122,
2022.
...
Edouard Grave, and Guillaume Lample. LLaMA: Open and efficient
foundation language models. arXiv preprint arXiv:2302.13971, 2023.
...
filter: A unified view of safety-critical control in autonomous systems.
