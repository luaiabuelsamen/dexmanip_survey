...
Procedural
Generation
Involve declarative knowledge
Involve procedural knowledge
LIBERO-Object
LIBERO-Spatial
LIBERO-Goal
LIBERO-100
...
(S, A, T , H, µ0, R). Here, S and A are the state and action spaces of the robot. µ0 is the initial
...
maxπ J(π) = Est,at∼π,µ0[
H
t=1 g(st)].
2.2
Lifelong Robot Learning Problem
In a lifelong robot learning problem, a robot sequentially learns over K tasks {T 1, . . . , T K} with a
single policy π. We assume π is conditioned on the task, i.e., π(· | s; T). For each task, T k ≡(µk
0, gk)
is defined by the initial state distribution µk
0 and the goal predicate gk.3 We assume S, A, T , H are
the same for all tasks. Up to the k-th task T k, the robot aims to optimize
max
π
JLRL(π) = 1
k
...
Due to the challenge of sparse-reward reinforcement learning, we
consider a practical alternative setting where a user would provide a small demonstration dataset
for each task in the sequence. Denote Dk = {τ k
i }N
i=1 as N demonstrations for task T k. Each
τ k
i = (o0, a0, o1, a1, . . . , olk) where lk ≤H. Here, ot is the robot’s sensory input, including the
perceptual observation and the information about the robot’s joints and gripper. In practice, the
observation ot is often non-Markovian. Therefore, following works in partially observable MDPs [25],
we represent st by the aggregated history of observations, i.e. st ≡o≤t ≜(o0, o1, . . . , ot). This
...
mixture model. Similarly, we assume {Dp : p < k} are not fully available when learning T k.
3
...
In order to accomplish a task such as put the
...
An important research question in LLDM is how to design
...
Given a policy architecture, it is crucial to determine
...
It is well-known that task curriculum influences policy
...
In practice, robots will be most likely pretrained on large
...
Human activities serve as a fertile source of
...
specify Objects And Layouts
a
8
(object
£2
5
black_bowl_1 - black boul.
—
plate1 - plate
-
)
...
.
Ee
(starget kitchen table)
Init Configuration
Goal States
.
-
“Open the top drawer of the
BOT
/
®
|
...
rd
Specify Task Goals
Specify Init Configuration
pe
(init
i
(0pen cabinet_1_top_drawer)
(on cabinet1 cabinet_region)’ 7
(In bowl_1 cabinet_1_top_drawer)
.
(on plate1 plate_init_region)
)
.
(on bowl1 bowlinit_region)’
)
:
*.pddi
)
...
<!-- page 5 (ocr) -->
form of language instructions. For example, we can generate an instruction “Open the drawer of the
cabinet” from the template “Open ...”.
Initial State Distribution (µ0)
To specify µ0, we first sample a scene layout that matches the
objects/behaviors in a provided instruction. For instance, a kitchen scene is selected for an instruction
Open the top drawer of the cabinet and put the bowl in it. Then, the details about µ0 are generated in
the PDDL language [43, 63]. Concretely, µ0 contains information about object categories and their
placement (Figure 2-(A)), and their initial status (Figure 2-(B)).
Goal Specifications (g)
Based on µ0 and the language instruction, we specify the task goal using
...
LIBERO-X
LIBERO-SPATIAL, LIBERO-OBJECT, and LIBERO-GOAL all have 10 tasks4
...
LIBERO-100
LIBERO-100 contains 100 tasks that entail diverse object interactions and versatile
...
<!-- page 6 (ocr) -->
architecture [75] uses a similar ResNet-based visual backbone, but a transformer decoder [66] as
...
Specifically, denote ci,j,e as the agent’s success rate on task j when it learned over i−1 previous tasks
and has just learned e epochs (e ∈{0, 5, . . . , 50}) on task i. Let ci,i be the best success rate over all
evaluated epochs e for the current task i (i.e., ci,i = maxe ci,i,e). Then, we find the earliest epoch e∗
i
in which the agent achieves the best performance on task i (i.e., e∗
i = arg mine ci,i,ei = ci,i), and
assume for all e ≥e∗
i , ci,i,e = ci,i.5 Given a different task j̸ = i, we define ci,j = ci,j,e∗
i . Then the
...
Table 1 reports the agent’s lifelong learning
...
<!-- page 8 (ocr) -->
used, we observe no significant performance difference between RESNET-T and VIT-T except on
...
Table 2 reports the lifelong learning per-
...
under equal sample sizes and unequal variance is applied with a p-value of 0.05.
...
impede the agent’s performance on LLDM problems (See Appendix E.2); and 4) ER, the rehearsal
...
Figure 4 shows the result of the study on Q4. For all experiments in
...
To better understand what type of knowledge the agent forgets during the
...
Pioneering work has adapted standard vision or language
...
continual learning with transformers. 2022.
11
...
<!-- page 13 (ocr) -->
[40] Zheda Mai, Ruiwen Li, Jihwan Jeong, David Quispe, Hyunwoo Kim, and Scott Sanner. Online
...
<!-- page 14 (ocr) -->
[56] Andrei A Rusu, Neil C Rabinowitz, Guillaume Desjardins, Hubert Soyer, James Kirkpatrick,
...
<!-- page 15 (ocr) -->
[71] Lemeng Wu, Bo Liu, Peter Stone, and Qiang Liu. Firefly neural architecture descent: a general
...
<!-- page 17 (ocr) -->
A
Implemented Neural Architectures and Lifelong Learning Algorithms
Neural Policy Arch.
RESNET-RNN
RESNET-T
VIT-T
...
with 8 16 CPUs for training and evaluation.
18
...
6
transformer_head_output_size
64
transformer_mlp_hidden_size
256
transformer_dropout
0.1
...
img_embed_size
128
spatial_transformer_num_layers
7
spatial_transformer_num_heads
8
spatial_transformer_head_output_size
120
spatial_transformer_mlp_hidden_size
256
spatial_transformer_dropout
0.1
spatial_down_sample_embed_size
64
temporal_transformer_input_size
null
temporal_transformer_num_layers
4
temporal_transformer_num_heads
6
temporal_transformer_head_output_size
64
temporal_transformer_mlp_hidden_size
256
temporal_transformer_dropout
0.1
temporal_transformer_max_seq_len
10
...
implemented.
ER
Experience Replay (ER) [13] is a rehearsal-based approach that maintains a memory buffer
...
EWC
Elastic Weight Consolidation(EWC) [33] is a regularization-based approach that add a
...
k−1,i
2 ,
where λ is a penalty hyperparameter, and the coefficient Fi is the diagonal of the Fisher information
matrix: Fk = Es∼DkEa∼pθ(·|s) (∇θk log pθk(a|s))2. In this work, we use the online update version
...
˜
Fk = γFk−1 + (1 −γ)Fk, where Fk = E(s,a)∼Dk (∇θk log pθk(a|s))2 and k is the task number. We
set γ = 0.9 and λ = 5 · 104.
PACKNET
PACKNET [41] is a dynamic architecture-based approach that aims to prevent changes
...
<!-- page 23 (ocr) -->
C.2
PDDL-based Scene Description File
...
( : f i x t u r e s
k i t c h e n _ t a b l e −k i t c h e n _ t a b l e
23
...
( : o b j _ o f _ i n t e r e s t
wooden_cabinet_1
akita_black_bowl_1
)
( : i n i t
(On akita_black_bowl_1
k i t c h e n _ t a b l e _ a k i t a _ b l a c k _ b o w l _ i n i t _ r e g i o n )
(On p l a t e _ 1
k i t c h e n _ t a b l e _ p l a t e _ i n i t _ r e g i o n )
(On wooden_cabinet_1
k i t c h e n _ t a b l e _ w o o d e n _ c a b i n e t _ i n i t _ r e g i o n )
)
( : goal
...
learning algorithm (refer to Figure15, 16, and 17)
25
...
algorithms and the three neural architectures. The best three results are highlighted in magenta (the
...
on task i throughout the LLDM procedure.
31
Pa tel stZL]
ETRE
IR INURE ED
...
throughout the LLDM procedure.
33
YRC FTF
£00 | OE] ERS E CREB
EN
hi th 0B de le edALA
pio LLL TER THEE
EE
kL
gt icl act ach EH CB COL CIN EI
§ FV
EN SEN EN EH El EE
...
We use a perturbation-based method [23] to extract
attention maps from agents. Given an input image I, the method applies a Gaussian filter to a pixel
...
or ViT encoder) πu(I) for image I. Then we define the saliency score as the Euclidean distance
between the latent representations of the original and the blurred images:
Sπ(i, j) = 1
2 πu(I) −πu(Φ(I, i, j))
2
.
(4)
Intuitively, Sπ(i, j) describes how much removing information from the region around location (i, j)
changes the policy. In other words, a large Sπ(i, j) indicates that the information around pixel (i, j)
...
pick up the orange
pick up the ketchup
pick up the alphabet
juice and place it
and place it in the
soup and place it in
in the basket
basket
the basket
or ojia yerha
