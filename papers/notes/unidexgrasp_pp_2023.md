...
- reward / objective (verbatim, paper + code):
  - paper (App. B.1) — the display equations, previously dropped by the PDF-to-markdown conversion (only prose survived), were recovered by OCR from papers/md/unidexgrasp_pp_2023.ocr.md (OCR text is noisier than the layout parse, symbols may be imperfect): reaching reward (Eq. 6) "rreach = −ωr‖xfinger −xobj‖2"; lifting reward (Eq. 7) "rlift = ωl∗(1 + az) if f = 2, 0 otherwise", with "f = Is(‖xfinger −xobj‖2 < λf1) + Is(dobj > λ0)" and "dobj = ‖xobj −xtarget‖2" ("a_z is the scaled force applied to the hand root along the z-axis (ω_l>0)"); moving reward (Eq. 8) "rmove = −ωmdobj + 1/(1+ωbdobj) if dobj < λ0, −ωmdobj otherwise" ("it will give a bonus term when the object is lifted very closely to the target"); and the total (Eq. 9) "r = rreach + rlift + rmove". The paper states "We use the non-goal-conditioned reward version in UniDexGrasp [70]... All the ω∗ here are hyper-parameters same with UniDexGrasp." Table 5 gives the weights ω_r=0.5, ω_l=0.1, ω_m=2, ω_b=10, matching the ω_r/ω_l/ω_m/ω_b symbols in the equations above.
...
- Caution for the survey, UPDATED: the reward equations (Eq. 6-9) were unrecoverable from the layout-parsed PDF extraction (converter dropped all LaTeX display equations) but have since been recovered by OCR from papers/md/unidexgrasp_pp_2023.ocr.md (OCR text is noisier than the layout parse, symbols may be imperfect) — see the reward/loss section above. The code (`compute_hand_reward`) remains useful as a numeric cross-check since the paper's hyperparameters ω∗ are symbolic-only.
