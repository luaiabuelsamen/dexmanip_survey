...
- r_fall: "a negative falling penalty when the object falls out of the palm" — exact functional form not recoverable: the Appendix has a "Falling Reward (Penalty)" sub-heading (Appendix D) but its formula is rendered as an unparsed figure/image in the source markdown, not text.
- r_work = "−⟨|τ|, |q̇_t|⟩", where τ is the PD controller's output torque at step t; "helps to improve the smoothness of finger motion."
- r_torque = "−∥τ∥", penalizing large torque. (Its "Torque Reward (Penalty)" appendix box is likewise an unextracted image.)
...
- The appendix "Rotation Reward" and "Velocity Reward" sub-headings (Appendix D) are also present with no extractable formula text (image only); the prose definitions above (from Sec IV.A.3) are the only verbatim source for r_rot and r_vel.
