...
Feeds the survey's sensing-modality comparison (touch-only vs. vision-based in-hand reorientation, e.g. against DeXtreme/dextreme_2022, which uses vision pose estimation and a locked wrist with no tactile sensing) and the reward-shaping comparison table, since this reward (finite-difference rotation angle rather than angular-velocity dot product) is explicitly presented as a deliberate departure from a cited prior reward ("[51] uses ⟨ω, k⟩"). No penetration handling here to compare against penetration-penalty methods — flag as a method with contact used purely for sensing, never for a physical/interpenetration penalty. Embodiment fields (palm-up/down, sim arm-mounting, real threshold θ_th default value) are genuinely silent in the source — but note r_fall is no longer among them: Appendix D Eq. 5 prints "rfall = −50.0." and is quoted in the reward block above (recovered by OCR from papers/md/rotating_without_seeing_2023.ocr.md; OCR text is noisier than the layout parse, symbols may be imperfect). The remaining silent fields are silent (not authoring gaps) — do not backfill these from other Allegro-hand papers when tabulating across notes.
