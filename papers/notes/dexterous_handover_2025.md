...
- reward: phase-switched, quoted below (block C). The PDF-to-markdown layout conversion did not extract the actual numbered equations (3)–(12) as text (they render as images in the source PDF); only the surrounding prose describing each term survived in `papers/md/dexterous_handover_2025.md`. The equations were subsequently recovered by OCR from papers/md/dexterous_handover_2025.ocr.md (OCR text is noisier than the layout parse, symbols may be imperfect) — see block C below for the full quoted set.
...
- Actual numeric equations (1)–(12), recovered by OCR from papers/md/dexterous_handover_2025.ocr.md (OCR text is noisier than the layout parse, symbols may be imperfect), quoted verbatim:
  - RL return objective (Eq. 1): "E[ T ∑ t=0 γtrt]" — i.e. E[Σ_{t=0}^T γ^t r_t], "where γ ∈[0,1] is the discounting factor over time."
  - Dual quaternion from rotation+translation (Eq. 2): "ˆq = qr +ε 1 2qr ·qt" — i.e. q̂ = q_r + ε·(1/2)q_r·q_t.
  - Base reward (Eq. 3): "rBASEt = ηt e−dt," — i.e. r_BASE_t = η_t · e^(−d_t).
  - Maneuver-phase modifier (Eq. 4): "mMANt = (dROBOTSt > dt)∧(dBACKt > dt)," — i.e. m_MAN_t = (d_ROBOTS_t > d_t) ∧ (d_BACK_t > d_t), with m_MAN_t ∈ {0,1}.
  - Approach-phase modifier (Eq. 5): "mt = 2[(dt < dt−1)∧mMANt]−1," where "(dt < dt−1) ∈{0,1} and mt ∈{−1,1}."
  - Contact-weighted reward weight (Eq. 6): "ηt = η0 ∑i 0⃗ci +1 ." — OCR renders this ambiguously (the denominator's summation bounds/notation are garbled — likely η_t = η0 / (Σ_i c⃗_i + 1), consistent with the prose "the more touches, the lower that value will be," but the exact denominator grouping could not be confirmed from this OCR pass).
  - Manipulation-phase reward (Eq. 7): "rMANIPt = α e−dTGTt ," where "α = 12 is the weight for the manipulation phase" — i.e. r_MANIP_t = α · e^(−d_TGT_t).
  - Final combined reward (Eq. 8): "rt = mt rBASEt +⃗c·⃗wc , if not (1G) rMANIPt +⃗c·⃗wc , otherwise" — i.e. a piecewise reward, r_t = m_t·r_BASE_t + c⃗·w⃗_c if not yet grasped (¬1_G), else r_t = r_MANIP_t + c⃗·w⃗_c, "where 1G ∈{0,1} indicates whether the agent has already grasped the object or not."
  - Dual-quaternion pose distance (Eq. 9): "dDQ = ||ˆqdiff −ˆI||2," i.e. d_DQ = ‖q̂_diff − Î‖₂.
  - Euler-angle distance (Eq. 10): "dEULER = ψ||⃗t1 −⃗t2||2 + µ||⃗e1 −⃗e2||," where "ψ = 2.1 and µ = 0.32 are scaling factors."
  - Rotation-matrix angular difference (Eq. 11): "θdiff = acos( trace(RT 13×3 ·R23×3)−1 2 )," i.e. θ_diff = acos((trace(R1ᵀ·R2) − 1)/2) (OCR splits the numerator/denominator of the fraction across lines with no visible division bar; the "/2" placement is inferred from the standard rotation-angle formula, not confirmed independently by this OCR pass).
  - Matrix-based distance (Eq. 12): "dMAT = ψ||⃗t1 −⃗t2||2 +β θdiff," where "ψ = 2.1 and β = 0.32 are scaling factors."
  (recovered by OCR from papers/md/dexterous_handover_2025.ocr.md; OCR text is noisier than the layout parse, symbols may be imperfect)
