...
- "Motion Deviation (MDev)": stable-contact condition (Eq. 2) and the per-tuple deviation formula (Eq. 3) recovered by OCR: "∀t ∈ {m, ..., n}, ||h_i^t − o_j^t|| ≤ α" (Eq. 2, stable-contact window definition) and "(1/(n−m)) Σ_{t=m+1}^{n} ||δh^_i^t − δo^_j^t||" (Eq. 3), "where δh^_i^t = h^_i^t − h^_i^{t−1} and δo^_j^t = o^_j^t − o^_j^{t−1}" (recovered by OCR from papers/md/arctic_2022.ocr.md; OCR text is noisier than the layout parse, symbols may be imperfect). For each longest stable-contact window (i, j, m, n) where hand vertex i and object vertex j stay "within a threshold alpha for every frame in the window": "we only consider longer motions by using windows with at least 0.5 second or 15 frames (i.e., n - m + 1 >= 15) and we choose alpha = 3mm"; averaged over windows.
