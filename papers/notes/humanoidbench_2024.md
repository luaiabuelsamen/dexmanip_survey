...
- reward: App. B-E defines `tol`, `height` (head z > 1.65, margin 0.4125), `upright` (z_proj > 0.9, margin 1.9), `stand = height x upright`, `e` (small control effort), `stable = stand x e`. walk: `R = stable x tol(vx, (1, inf), 1)`, terminate after 1000 steps or z_pelvis < 0.2. run: `stable x tol(vx, (5, inf), 5)`. stand: `stable x mean(still_x, still_y)`. push: `d_goal = d(box, destination)`, `success = 1[d_goal < 0.05]`, `d_hand = d(box, hand_left)`, "by default alpha_s = 1000, alpha_t = 1, alpha_h = 0.1"; the equation itself, lost by the converter, is recovered as `R(s, a) = αs · success −αt · dgoal −αh · dhand where by default αs = 1000, αt = 1, αh = 0.1.` (recovered by OCR from papers/md/humanoidbench_2024.ocr.md; OCR text is noisier than the layout parse, symbols may be imperfect); terminate after 500 steps or d_goal < 0.05. package: `success = 1[d(package, destination) < 0.1]`, reward `... + stable + height_package + 1000 * success`, terminate after 1000 steps or success. cube: the orientation and `proximity_cube` equations, lost by the converter, are recovered as `• orientation = 1 2[(quatcube,left −quattarget)2 +(quatcube,right −quattarget)2]`, `• proximitycube = 1 2[tol(d(cubeleft, handleft), (0, 0), 0.5) + tol(d(cuberight, handright), (0, 0), 0.5)]` and the task reward `R =0.2 · (stable × still) + 0.5 · orientation + 0.3 · proximitycube` (recovered by OCR from papers/md/humanoidbench_2024.ocr.md; OCR text is noisier than the layout parse, symbols may be imperfect) — the OCR renders the typeset fraction 1/2 as a bare `1` on its own line followed by `2[...]`, and gives no `‖·‖` around the quaternion difference, so the exact norm/squaring convention of the orientation term is still not certain from this text. Terminate after 500 steps or z_pelvis < 0.5 or either cube z < 0.5. kitchen: "sparse reward is the number of subtasks completed", max 4. basketball: 1000 sparse reward when d(ball, basket) <= 0.05. cabinet: three subtasks with `R_3 = 0.2 * stable + 0.8 * r_3`.
