...
- observation / action: Sec. 6.2 "Input Modeling" (vision-action; VLA with 2D or 3D input; tactile-based; extra modalities). Sec. 2.2.2 gives the MDP five-tuple, recovered by OCR: "An MDP is defined as a five-tuple: M = ⟨S, A, P, R, 𝛾⟩, (1) where • S denotes the state space containing all possible environment and robot states; • A denotes the action space containing the executable actions available to the robot; • P(𝑠′ | 𝑠, 𝑎) denotes the transition probability of reaching state 𝑠′ after executing action 𝑎 in state 𝑠; • R : S×A →ℝ denotes the reward function specifying the expected immediate reward obtained by executing action 𝑎 in state 𝑠; • 𝛾∈[0, 1] denotes the discount factor that balances short-term and long-term rewards." (equation image in the layout parse; recovered by OCR from papers/md/bai_unified_manip_survey_2025.ocr.md; OCR text is noisier than the layout parse, symbols may be imperfect)
