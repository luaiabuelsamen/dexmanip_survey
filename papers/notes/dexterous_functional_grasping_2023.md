...
- reward or loss (quoted, Sec. 2.2 "Rewards"): "a simple reward function that is a combination of two terms r_threshold and r_hand-obj."
  ...
  - r_hand-obj: described only in prose as "a sum of exponentials and an L2 distance to incentivize the object to be close to the palm of the hand," with constants d1=10cm, d2=5cm, d3=1cm given in text — but the actual equation is not present in the extracted markdown (it appears to have been an inline image/equation lost in PDF→markdown conversion). The functional form cannot be verified.
...
Reward block (C) is only partially sourced against HOWTO_METHOD's "quote every term with its weight" requirement: the binary threshold term and the overall linear combination (r_hand-obj + 0.1·r_threshold + 1) are verbatim from the text, but the exact functional form of r_hand-obj — the "sum of exponentials and an L2 distance" — did not survive PDF→markdown conversion (likely an inline equation image), and there is no code to recover or cross-check it. Flag this if the survey later compares reward-shaping density across papers.
