...
commit: 146b34afef8c1a32adeef7e981c070109f225c87
...
hot3d/
  .gitattributes
  .gitignore
  HOT3D_Tutorial.ipynb
  Hot3DVisualizer.py
  clips/
    README.md
    bop_format_converters/
    clip_util.py
    vis_clips.py
  data_downloader/
    dataset_download_status_manager.py
    dataset_downloader_base.py
    dataset_downloader_base_main.py
    utils.py
  data_loaders/
    AlignedBox2d.py
    AriaDataProvider.py
    HandBox2dDataProvider.py
    HandDataProviderBase.py
    HeadsetPose3dProvider.py
    ManoHandDataProvider.py
    ObjectBox2dDataProvider.py
    ObjectPose3dProvider.py
    PathProvider.py
    QuestDataProvider.py
    UmeTrackHandDataProvider.py
    constants.py
    frameset.py
    hand_common.py
    headsets.py
    io_utils.py
    loader_hand_poses.py
    loader_masks.py
    loader_object_library.py
    loader_poses_utils.py
    mano_layer.py
    pose_utils.py
    pytorch3d_rotation/
    tests/
...
### hot3d/data_loaders/HandBox2dDataProvider.py
...
class HandBox2dProvider()
    def __init__(self, box2d_trajectory_collection)
    def get_timestamp_ns_list(self, stream_id)
    def stream_ids(self)
    def get_data_statistics(self)
    def get_bbox_at_timestamp(self, stream_id, timestamp_ns, time_query_options, time_domain)
def parse_box2ds_from_csv_reader(csv_reader)
def load_box2d_trajectory_from_csv(filename)
```

### hot3d/data_loaders/HandDataProviderBase.py

```
class HandPose3dCollectionWithDt()
class HandDataProviderBase()
    def __init__(self)
    def _init_hand_poses(self, hand_pose_trajectory_filepath)
    def timestamp_ns_list(self)
    def get_data_statistics(self)
    def get_pose_at_timestamp(self, timestamp_ns, time_query_options, time_domain, acceptable_time_delta)
    def get_hand_mesh_vertices(self, hand_wrist_data)
    def get_hand_mesh_faces_and_normals(self, hand_wrist_data)
    def get_hand_landmarks(self, hand_wrist_data)
    def normalized(vecs, axis, add_const_to_denom)
    def get_triangular_mesh_normals(vertices, triangles)
```

### hot3d/data_loaders/ManoHandDataProvider.py

```
class MANOHandDataProvider(HandDataProviderBase)
    def __init__(self, hand_pose_trajectory_filepath, mano_layer)
    def get_hand_mesh_vertices(self, hand_wrist_data)
    def get_hand_mesh_faces_and_normals(self, hand_wrist_data)
    def get_hand_landmarks(self, hand_wrist_data)
```

### hot3d/data_loaders/UmeTrackHandDataProvider.py

```
class UmeTrackHandModelData()
def from_dict(j)
def load_hand_model_from_file(filename)
class UmeTrackHandDataProvider(HandDataProviderBase)
    def __init__(self, hand_pose_trajectory_filepath, hand_profile_filepath)
    def get_hand_mesh_vertices(self, hand_wrist_data)
    def get_hand_mesh_faces_and_normals(self, hand_wrist_data)
    def get_hand_landmarks(self, hand_wrist_data)
def skin_landmarks(hand_model, joint_angles, wrist_transforms)
def skin_vertices(hand_model, joint_angles, wrist_transforms)
```

### hot3d/data_loaders/hand_common.py

```
class LANDMARK(Enum)
```

### hot3d/data_loaders/loader_hand_poses.py
...
class HandPose()
    ...
    def handedness_label(self)
class HandPose3dCollection()
    """Class to store the Hand poses for a given timestamp"""
def _get_hand_pose(handedness, hand_poses_json)
def _get_joint_angles(handedness, hand_poses_json)
def parse_hand_poses_from_fileobject(fileobject)
def load_hand_poses(filename)
def load_hand_pose_as_json_lines(filename)
def load_mano_shape_params(filename)
```

### hot3d/data_loaders/tests/test_HandBox2dDataProvider.py

```
class TestHandBox2dDataProvider(TestCase)
    def setUp(self)
    def test_provider_aria_recording(self)
```

### hot3d/data_loaders/tests/test_HandDataProvider.py

```
class TestHandDataProvider(TestCase)
    def setUp(self)
    def test_file_exists(self)
    def test_provider_mano_hands(self)
    def test_provider_hand_umetrack(self)
    def hand_provider_test(self, hand_data_provider)
```