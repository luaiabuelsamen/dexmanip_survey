# geometric_retargeting_2025

source: https://github.com/facebookresearch/GeoRT


commit: fb2075e2a3272be372016580c0b6ebd6120228f0
...
class HandKinematicModel()
    def __init__(self, scene, render, hand, hand_urdf, n_hand_dof, base_link, joint_names, kp, kd)
    def __del__(self)
    def get_n_dof(self)
    ...
    def keypoint_from_qpos(self, qpos, ret_vec)
...
class GeoRTTrainer()
    ...
    def get_robot_kinematics_dataset_path(self, postfix)
    def get_keypoint_info(self)
    def generate_robot_kinematics_dataset(self, n_total, save)
    def get_fk_checkpoint_path(self)
    def get_robot_neural_fk_model(self, force_train)
    def train(self, human_data_path)
```

### geort/utils/config_utils.py
...
### geort/utils/hand_utils.py

```
def check_contact(scene, actors1, actors2, impulse_threshold)
def get_active_joint_indices(articulation, joint_names)
def get_active_joints(articulation, joint_names)
def get_entity_by_name(entities, name, is_unique)
```