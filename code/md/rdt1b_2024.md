...
commit: cd79363a1387e8f81c7724d070ef7e45fd23150f
...
configs/
  base.yaml
  calvin_rel_traj_location_bounds_task_ABC_D.json
  dataset_control_freq.json
  dataset_img_keys.json
  dataset_stat.json
  finetune_datasets.json
  finetune_sample_weights.json
  pretrain_datasets.json
  pretrain_sample_weights.json
  state_vec.py
  zero2.json
eval_sim/
  eval_dp.py
...
## Python signatures and reward/observation bodies (9 files)
...
This dataset will load data from the buffer directory."""
    def __init__(self, config, tokenizer, image_processor, num_cameras, img_history_size, image_size, auto_adjust_image_brightness, image_aug, dataset_type, cond_mask_prob, cam_ext_mask_prob, state_noise_snr, use_hdf5, use_precomp_lang_embed)
...
```
def image_corrupt(image)
```

### train/sample.py

```
def log_sample_res(text_encoder, vision_encoder, rdt, args, accelerator, weight_dtype, dataset_id2name, dataloader, logger)
```

### train/train.py

```
def save_model_card(repo_id, base_model, repo_folder)
