...
commit: c8f03f48af692657d3060c19588038c7220e9af9
...
def state_to_eep(xyz_coor, zangle)
...
https://github.com/rail-berkeley/bridge_data_robot"""
    def __init__(self, widowx_client, cfg, im_size, blocking)
...
def get_vla_action(vla, processor, base_vla_name, obs, task_label, unnorm_key, center_crop)
```
...
class PrismaticConfig(PretrainedConfig)
    def __init__(self, vision_backbone_id, llm_backbone_id, arch_specifier, use_fused_vision_backbone, image_resize_strategy, text_config, llm_max_length, pad_token_id, pad_to_multiple_of, output_projector_states)
class OpenVLAConfig(PrismaticConfig)
    def __init__(self, norm_stats, n_action_bins)
```
...
def get_train_strategy(train_strategy, vlm, device_id, stage, epochs, max_steps, global_batch_size, per_device_batch_size, learning_rate, weight_decay, max_grad_norm, lr_scheduler_type, warmup_ratio, enable_gradient_checkpointing, enable_mixed_precision_training, reduce_in_full_precision, mixed_precision_dtype, worker_init_fn)
```
...
class Tracker(Protocol)
    ...
    def write(self, global_step, metrics)
    def finalize(self)
class JSONLinesTracker()
    def __init__(self, run_id, run_dir, hparams)
    def write_hyperparameters(self)
    def write(self, _, metrics)
    def finalize(self)
class WeightsBiasesTracker()
    def __init__(self, run_id, run_dir, hparams, project, entity, group)
    def initialize(self)
    def write_hyperparameters(self)
    def write(self, global_step, metrics)
    def finalize()
class Metrics()
    def __init__(self, active_trackers, run_id, run_dir, hparams, stage, wandb_project, wandb_entity, grad_accumulation_steps, window_size)
    def log(self, global_step, metrics)
    def get_status(self, loss)
    def commit(self)
    def push(self)
    def finalize(self)
class VLAMetrics()
    def __init__(self, active_trackers, run_id, run_dir, hparams, wandb_project, wandb_entity, grad_accumulation_steps, window_size, resume_step, resume_epoch)
    def log(self, global_step, metrics)
    def get_status(self, loss)
    def commit(self)
    def commit_for_dataset(self, dataset_name)
    def push(self)
    def finalize(self)
```
...
class TrainingStrategy(ABC)
    def __init__(self, vlm, device_id, stage, epochs, max_steps, global_batch_size, per_device_batch_size, learning_rate, weight_decay, max_grad_norm, lr_scheduler_type, warmup_ratio, enable_gradient_checkpointing, enable_mixed_precision_training, reduce_in_full_precision, mixed_precision_dtype, worker_init_fn)
    def save_checkpoint(self, run_dir, global_step, epoch, train_loss, only_trainable)
    def run_setup(self, run_dir, n_train_examples)
    def clip_grad_norm(self)
    def run_training(self, dataset, collator, metrics, stage, batch_construction_strategy, seed)
    def run_vla_training(self, vla_dataset, collator, action_tokenizer, metrics, save_interval, save_full_model)
```
...
class FSDPStrategy(TrainingStrategy)
    def __init__(self, vlm, device_id, stage, epochs, max_steps, global_batch_size, per_device_batch_size, learning_rate, weight_decay, max_grad_norm, lr_scheduler_type, warmup_ratio, enable_gradient_checkpointing, enable_mixed_precision_training, reduce_in_full_precision, mixed_precision_dtype, worker_init_fn, sharding_strategy, state_dict_type)
    def save_checkpoint(self, run_dir, global_step, epoch, train_loss, only_trainable)
    def run_setup(self, run_dir, n_train_examples)
    def clip_grad_norm(self)
```
...
class ActionTokenizer()
    def __init__(self, tokenizer, bins, min_action, max_action)
...
Usage:
    python vla-scripts/extern/convert_openvla_weights_to_hf.py         --openvla_model_path_or_id <PATH TO PRISMATIC TRAINING RUN DIR>         -"""
class HFConvertConfig()
    def __post_init__(self)
def _ls_new_forward(self, x)
def ls_apply_patch(ls_module)
def remap_state_dicts_for_hf(prismatic_vision_backbone_state_dict, projector_state_dict, llm_backbone_state_dict, use_fused_vision_backbone)
def convert_openvla_weights_to_hf(cfg)
```
...
def get_openvla_prompt(instruction)
def verify_openvla()
```
