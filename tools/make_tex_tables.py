...
# contribution and are read, not consulted; and the task-family table, which is written by
# sections/02_taxonomy.tex rather than here.
#
...
WITHDRAWN = {
    ...
    "table9_surveys.tex":
        "what each predecessor survey covers. Consulted. paper/tables/table10_surveys.md",
...
def table1():
    ...
    note = (r"A row whose joint count no source states sorts last rather than being ranked by a "
            ...
            r"\texttt{corpus/rows/}. \emph{force N} is not a ranking: read \emph{force kind} "
...
def table7():
    ...
    cap = (r"The proposed evaluation protocol, this survey's own contribution and the reason it "
           r"is in the body rather than in the repository. No cell is \na: every cell is a "
           r"proposal rather than a reading of a source, and this is the one table in the paper "
           r"that no corpus row can fill.")
    note = (r"Two columns rather than one: four columns of clauses do not fit 252\,pt without "
            r"breaking words. The \emph{how} and \emph{why} columns of the long form, which carry "
            r"the design of each measurement and the reason for each count, are derived in "
            r"Section~VII and generated by \texttt{tools/make\_eval\_tables.py}; a paragraph of "
            rf"rationale does not belong in a float. The bill for two policies on three tasks is "
            rf"{matched} matched plus {unseen} unseen real rollouts, against {old} on the "
            rf"independent-arm count the paired design replaces.")
    return write_table("table7_protocol.tex", "tab:protocol", cap, cols, body, note=note)
